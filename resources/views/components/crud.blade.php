...
<div class="flex justify-center">
    <a href="{{route("crud.create", $resource)}}"
       class="btn bg-boton-primary hover:bg-boton-secondary border-none m-5 ">{{__("Añadir")}} {{$table}}</a>
</div>
            ...
            @foreach($rows as $row)
                <tr class="lg:text-sm ">
                    @foreach($fields as $attribute => $value)
                        <td class="border-b-gray-400 text-md text-center">
                            <div class="{{ in_array($attribute,['password']) ? 'max-w-3xs truncate text-center' : '' }}">
                                {{$row->$attribute}}
                            </div>
                        </td>
                    @endforeach
                    ...
                    <td class="border-b-gray-400">
                        <form action="{{route("crud.destroy",[$resource, $row->id])}}?page={{$page}}" method="POST">
                            @csrf
                            @method('DELETE')
                            <input type="button" value="Borrar" class="btn btn-error text-white"
